...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  if (empty($_POST['loan_number'])) {
    $_SESSION['error_message'] = 'Loan number is required.';
    header('Location: ' . $_SERVER['PHP_SELF']);
    exit();
  } else {
    $loanPrint = getLoanPrint($pdo, $_POST['loan_number']);
    if (empty($loanPrint)) {
      $_SESSION['error_message'] = 'No Loans found for the provided loan number.';
      header('Location: ' . $_SERVER['PHP_SELF']);
      exit();
    }
  }
...
<body>
  ...
  <div class="pages-content">
    <form method="POST" action="">
      <h1>Loans Receipts</h1>
      <input type="text" name="loan_number" placeholder="Enter Loan Number">
    ...
    <div class="content" id="printContent">
      <div class="print">
        ...
        <?php if (isset($loanPrint) && !empty($loanPrint)) : ?>
          <?php foreach ($loanPrint as $loan) : ?>
            <div class="print">
              <p>Loan Receipt Number: <?php echo htmlspecialchars($loan['loan_number']); ?></p>
            </div>

            <div class="print">
              <p>Invoice Made By: <?php echo htmlspecialchars($loan['loaned_by']); ?> </p>
            </div>

            <div class="print">
              <p>
                Customer Name: <?php echo htmlspecialchars($loan['customer_name']); ?>
              <p>
            </div>

            <div class="print">
              <p>Loan Date: <?php echo date('d/m/Y H:i:s', strtotime($loan['loan_date'])); ?></p>
            </div>
            ...
            <table class="printTable">
              <tr>
                ...
                <th>Unit Price</th>
                <th>Discount %</th>
                <th>Total Credit(GNF)</th>
              </tr>
              <tr>
                <td><?php echo htmlspecialchars($loan['product_name']); ?></td>
                <td><?php echo htmlspecialchars($loan['loan_quantity']); ?></td>
                <td><?php echo htmlspecialchars($loan['unit_price']); ?></td>
                <td><?php echo htmlspecialchars($loan['discount']); ?></td>
                <td><?php echo htmlspecialchars($loan['invoice_amount']); ?></td>
              </tr>
            </table>
            <hr>
          <?php endforeach; ?>
